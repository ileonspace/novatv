...
export async function getCachedLiveChannels(
  ...
  if (!cachedLiveChannels[cacheKey]) {
    // 清理旧的纯 key 缓存（配置变更前）
    if (cachedLiveChannels[key] && key !== cacheKey) {
      delete cachedLiveChannels[key];
    }
    ...
    // refreshLiveChannels 存到 liveInfo.key，迁移到 cacheKey
    if (cachedLiveChannels[liveInfo.key] && cacheKey !== liveInfo.key) {
      cachedLiveChannels[cacheKey] = cachedLiveChannels[liveInfo.key];
      delete cachedLiveChannels[liveInfo.key];
    }
  }
  return cachedLiveChannels[cacheKey] || null;
}
