...
// ---- 常量 ----
const PLAY_RECORDS_KEY = 'novatv_play_records';
const FAVORITES_KEY = 'novatv_favorites';
const SEARCH_HISTORY_KEY = 'novatv_search_history';

// 缓存相关常量
const CACHE_PREFIX = 'novatv_cache_';
...
class HybridCacheManager {
  ...
  private clearAllCache(): void {
    ...
    keys.forEach((key) => {
      if (key.startsWith('novatv_cache_')) {
...
export async function getSkipConfig(
  ...
  // localStorage 模式
  try {
    const raw = localStorage.getItem('novatv_skip_configs');
    if (!raw) return null;
...
export async function saveSkipConfig(
  ...
  try {
    ...
    configs[key] = config;
    localStorage.setItem('novatv_skip_configs', JSON.stringify(configs));
    window.dispatchEvent(
      new CustomEvent('skipConfigsUpdated', {
        detail: configs,
      })
