...
import { getApiSitesFromRequest, getConfig } from '@/lib/config';
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 生成建议
    const suggestions = await generateSuggestions(config, query, authInfo.username, request);
...
async function generateSuggestions(config: AdminConfig, query: string, username: string, request: NextRequest): Promise<
  Array<{
    text: string;
...
> {
  ...
  const apiSites = await getApiSitesFromRequest(request, username);
