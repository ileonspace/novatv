...
import {
  ...
  getDoubanList,
  getDoubanRecommends,
  peekDoubanCategories,
  peekDoubanList,
  peekDoubanRecommends,
...
function DoubanPageClient() {
  ...
  // 根据当前参数构造对应的数据请求（返回 Promise<DoubanResult>）
  // forceRefresh=true 时绕过缓存强制走网络（用于缓存命中后的后台刷新）
  const buildDataRequest = useCallback(
    (pageStart: number, forceRefresh = false) => {
      if (type === 'custom') {
        ...
        return GetBangumiCalendarData().then((calendarData) => {
      ...
      return getDoubanCategories(getRequestParams(pageStart), forceRefresh);
    },
    [
      type,
      primarySelection,
  ...
  const loadInitialData = useCallback(async () => {
    ...
    // 先尝试读取缓存：命中立即显示，避免切换分类空白
    let cachedData: DoubanResult | null = null;
    try {
      if (type === 'custom') {
        const selectedCategory = customCategories.find(
          (cat) =>
        ...
        if (selectedCategory) {
          cachedData = await peekDoubanList({
            tag: selectedCategory.query,
            type: selectedCategory.type,
            pageLimit: 25,
            pageStart: 0,
        ...
        cachedData = await peekDoubanRecommends({
          kind: primarySelection === '番剧' ? 'tv' : 'movie',
          pageLimit: 25,
          pageStart: 0,
          category: '动画',
        ...
        if (type !== 'anime') {
          cachedData = await peekDoubanCategories(getRequestParams(0));
        }
      }
    } catch (e) {
      // 缓存读取失败不影响正常加载
    ...
    // 缓存命中：立即填充，然后后台静默刷新
    if (cachedData && cachedData.code === 200 && cachedData.list?.length) {
      const currentSnapshot = { ...currentParamsRef.current };
      if (isSnapshotEqual(requestSnapshot, currentSnapshot)) {
        setDoubanData(cachedData.list);
        setCurrentPage(0);
        setHasMore(true);
        setIsLoadingMore(false);
        setLoading(false);

        // 后台刷新缓存，不阻塞显示（forceRefresh=true 绕过缓存强制走网络，
        // 才能真正拿到最新数据刷新屏幕）
        buildDataRequest(0, true)
          .then((fresh) => {
            if (fresh.code === 200 && fresh.list?.length) {
              const freshSnapshot = { ...currentParamsRef.current };
              if (isSnapshotEqual(requestSnapshot, freshSnapshot)) {
                setDoubanData(fresh.list);
                setHasMore(fresh.list.length !== 0);
              }
            }
          })
          .catch(() => undefined);
    ...
    try {
      ...
      const data = await buildDataRequest(0);
    ...
    if (currentPage > 0) {
      const fetchMoreData = async () => {
        ...
        try {
          ...
          const data = await buildDataRequest(currentPage * 25);
