  ...
  useEffect(() => {
    ...
    if (query) {
      ...
      if (currentFluidSearch) {
        // 流式搜索：打开新的流式连接
        const es = new EventSource(`/api/search/ws?q=${encodeURIComponent(trimmed)}${getConfigUrlSuffix()}`);
        ...
        // 传统搜索：使用普通接口
        fetch(`/api/search?q=${encodeURIComponent(trimmed)}${getConfigUrlSuffix()}`)
