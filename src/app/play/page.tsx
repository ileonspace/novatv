...
import { getConfigUrlSuffix } from '@/lib/config.client';
import {
  deleteFavorite,
  deletePlayRecord,
  deleteSkipConfig,
...
function PlayPageClient() {
  ...
  useEffect(() => {
    const fetchSourceDetail = async (
      ...
      try {
        const detailResponse = await fetch(
          `/api/detail?source=${source}&id=${id}${getConfigUrlSuffix()}`
        );
        if (!detailResponse.ok) {
          throw new Error('获取视频详情失败');
        }
        const detailData = (await detailResponse.json()) as SearchResult;
        await cacheSet(cacheKey, detailData);
    ...
    // NovaTV 流式搜索：逐源返回，第一个可用源立即回调（秒进播放）
    const streamingSearch = (
      query: string,
      onSource: (results: SearchResult[]) => void
    ): Promise<SearchResult[]> => {
      return new Promise((resolve) => {
        const allResults: SearchResult[] = [];
        let finished = false;
        const es = new EventSource(
          `/api/search/ws?q=${encodeURIComponent(query)}${getConfigUrlSuffix()}`
        );
        searchEsRef.current = es; // 存 ref，卸载时关闭
        es.onmessage = (event) => {
          if (!event.data || finished) return;
          try {
            const payload = JSON.parse(event.data);
            if (
              payload.type === 'source_result' &&
              Array.isArray(payload.results) &&
              payload.results.length > 0
            ) {
              allResults.push(...payload.results);
              // 每个源到达立即通知（首个可播源立即播放），但继续收集全部源
              onSource(payload.results);
    ...
    const initAll = async () => {
      ...
      // 应用详情：设置数据并初始化播放器
      const applyDetail = (detailData: SearchResult) => {
        // 无可用播放地址时提示，避免静默空白
        if (!detailData.episodes || detailData.episodes.length === 0) {
          setError('该片源暂无可用播放地址');
          setLoading(false);
          return;
        }
      ...
      // 有明确 source/id → 直接取详情（秒进播放）
      if (currentSource && currentId && !needPreferRef.current) {
        const detailRes = await fetchSourceDetail(currentSource, currentId);
        if (detailRes.length === 0) {
          setError('未找到匹配结果');
          setLoading(false);
          return;
        }
        applyDetail(detailRes[0]);

        // 后台搜索收集全部源，填充换源列表（不阻塞播放）
        const searchKeyword =
      ...
      const allSources = await streamingSearch(
    ...
    // 卸载时关闭流式搜索 EventSource，避免连接泄漏
    return () => {
      if (searchEsRef.current) {
  ...
  useEffect(() => {
    ...
    initFromHistory();
    // NovaTV：依赖 source/id，从搜索/豆瓣卡片进入（异步设置源后）也能恢复进度
  }, [currentSource, currentId]);
  ...
  // NovaTV：后台测速所有源 → 按速度排序 → 返回最佳源
  const optimizeAndSortSources = async (
    sources: SearchResult[]
        ...
        try {
          ...
          const url =
            source.episodes.length > 1
              ? source.episodes[1]
              : source.episodes[0];
          const testResult = await getVideoResolutionFromM3u8(url);
    ...
    >();
    results.forEach((r) => {
      if (r) infoMap.set(`${r.source.source}-${r.source.id}`, r.testResult);
    });
    setPrecomputedVideoInfo(infoMap);

    // 质量分：4K=4 > 1080p=3 > 720p=2 = 流畅(2，检测不到的中性) > 480p=1 > 失败(0)
    const qualityScore = (q: string | undefined): number => {
      if (q === '4K') return 4;
      if (q === '1080p') return 3;
      if (q === '720p' || q === '流畅') return 2;
      if (q === '480p') return 1;
      return 0;
    };
    // 质量优先，同质量按延迟升序；检测不到质量的源（流畅=中性）不排后
    const sorted = [...sources].sort((a, b) => {
      const ia = infoMap.get(`${a.source}-${a.id}`);
      const ib = infoMap.get(`${b.source}-${b.id}`);
      const qa = qualityScore(ia?.quality);
      const qb = qualityScore(ib?.quality);
      if (qa !== qb) return qb - qa;
      const pa = ia?.pingTime;
      const pb = ib?.pingTime;
    ...
    // 返回最佳源（质量最高，同质量延迟最低；检测不到的"流畅"不排后）
    const best = results
      .filter(Boolean)
      .sort((x: any, y: any) => {
        const qx = qualityScore(x.testResult.quality);
        const qy = qualityScore(y.testResult.quality);
        if (qx !== qy) return qy - qx;
        return x.testResult.pingTime - y.testResult.pingTime;
      })[0];
  ...
  useEffect(() => {
    return () => {
      // NovaTV 修复：组件卸载（SPA 返回）时销毁播放器，避免卡死/内存泄漏
      cleanupPlayer();
    };
  }, []);
