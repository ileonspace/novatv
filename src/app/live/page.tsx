...
import { getConfigParam, getConfigUrlSuffix } from '@/lib/config.client';
import {
  deleteFavorite,
  generateStorageKey,
...
function LivePageClient() {
  ...
  // 当前时间（每分钟更新，稳定引用避免子组件定时器反复重置）
  const [epgNow, setEpgNow] = useState(() => new Date());
  useEffect(() => {
    const t = setInterval(() => setEpgNow(new Date()), 60000);
  ...
  // EPG 请求序号（快速切频道时丢弃过期响应）
  const epgSeqRef = useRef(0);
  ...
  const fetchChannels = async (source: LiveSource) => {
    try {
      ...
      const response = await fetch(
        `/api/live/channels?source=${source.key}${getConfigUrlSuffix()}`
      );
      if (!response.ok) {
  ...
  const handleChannelChange = async (channel: LiveChannel) => {
    ...
    if (channel.tvgId && currentSource) {
      ...
      try {
        ...
        if (seq === epgSeqRef.current) {
          console.error('获取节目单信息失败:', error);
        }
      } finally {
        if (seq === epgSeqRef.current) {
          setIsEpgLoading(false); // 仅最新请求结束加载状态
        }
      }
    } else {
    ...
    constructor(config: any) {
      ...
      this.load = function (context: any, config: any, callbacks: any) {
        ...
        try {
          ...
          url.searchParams.set('novatv-source', currentSourceRef.current?.key || '');
