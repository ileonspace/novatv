...
import { getShowLive, setShowLive } from '@/lib/config.client';
...
export const UserMenu: React.FC = () => {
  ...
  const [isConfigImporterOpen, setIsConfigImporterOpen] = useState(false);
          ...
          <div>
            ...
            {isManageOpen && (
              <div className='ml-4 border-l border-gray-200 dark:border-gray-700 pl-2'>
                {/* 导入配置 */}
                <button
                  onClick={() => setIsConfigImporterOpen(true)}
                  className='w-full px-3 py-2 text-left flex items-center gap-2.5 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors text-sm'
                >
  ...
  return (
    <>
      ...
      {/* 导入配置面板 */}
      <ConfigImporter
        isOpen={isConfigImporterOpen}
        onClose={() => setIsConfigImporterOpen(false)}
      />
    </>
  );
};
