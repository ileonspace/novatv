...
const MultiLevelSelector: React.FC<MultiLevelSelectorProps> = ({
  ...
  return (
    <>
      {/* 胶囊样式筛选栏（移动端可横向滚动） */}
      <div className='relative flex overflow-x-auto scrollbar-hide rounded-full p-0.5 sm:p-1 bg-transparent gap-1 sm:gap-2 max-w-full'>
                ...
                {categories
                  ...
                  ?.options.map((option) => (
                    <button
                      ...
                      className={`px-2.5 py-2 sm:px-3 sm:py-2 text-[13px] sm:text-[13px] rounded-lg transition-all duration-200 text-left ${isOptionSelected(activeCategory, option.value)
