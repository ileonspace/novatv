      ...
      <div className='relative inline-flex rounded-full p-0.5 sm:p-1 bg-transparent gap-1 sm:gap-2'>
        {categories.map((category) => (
          <div key={category.key} ref={(el) => { categoryRefs.current[category.key] = el; }} className='relative'>
            <button
              ...
              className={`relative z-10 px-1.5 py-0.5 sm:px-2 sm:py-1 md:px-4 md:py-2 text-[13px] sm:text-[13px] font-medium rounded-full transition-all duration-200 whitespace-nowrap ${activeCategory === category.key
        ...
        <div className='relative'>
          <button
            ...
            className={`relative z-10 px-1.5 py-0.5 sm:px-2 sm:py-1 md:px-4 md:py-2 text-[13px] sm:text-[13px] font-medium rounded-full transition-all duration-200 whitespace-nowrap ${mergedValues.yearOrder === 'none'
              ? 'text-gray-700 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-100 cursor-pointer'
              : 'text-green-600 hover:text-green-700 dark:text-green-400 dark:hover:text-green-300 cursor-pointer'
              }`}
            ...
            <div className='grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-1 sm:gap-2'>
              {categories.find((cat) => cat.key === activeCategory)?.options.map((option) => (
                <button
                  ...
                  className={`px-2 py-1.5 sm:px-3 sm:py-2 text-[13px] sm:text-[13px] rounded-lg transition-all duration-200 text-left ${isOptionSelected(activeCategory, option.value)
